...
from ..mutils import show_data, accuracy
...
class LogRecorder(object):
    ...
    def print_log(self, iter_log_period, iters, mean_loss, record_detail, data, show_clip, **kwargs):
        ...
        if iter_log_period is not None and iters % iter_log_period == 0:
            ...
            show_data(data[:n], self.mean, self.std, show_clip)
        if iter_log_period is not None and iters % 5 == 0:
            print('iter: {} loss: {:.4f}'.format(iters, mean_loss), end=' ')
            if record_detail:
                n = data.shape[0]
                print("acc: {} MSE: {:.2f} result_score: {:.3f} label_score: {:.3E}".format(self.acc[-1]/n, self.sse[-1]/ n, np.exp(self.log_result_score[-1]/n), np.exp(self.log_label_score[-1]/n)), end=' ')
            print("")
            # print self.result[-1]
